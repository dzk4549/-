...
//	}
	...
	//由仓库id查询货物信息
	public function getGoodsByWid($Wid){
		$sql = 'SELECT * FROM storage_goods WHERE GWid = '. $Wid;
	...
	public function getGoods($Gid = 0){
		$sql = $Gid == 0 ? 'SELECT * FROM storage_goods' : 'SELECT * FROM storage_staff WHERE Gid = '. $Gid;
		$goods = Yii::app()->db->createCommand($sql)->queryAll();
		return $goods;
	}
}
